...
func (s *MasterSuite) TestSerialAssignAndCompleteJobs() {
	...
	// Fetch the worker object from the unassigned queue and push it back there again.
	...
	for i := 0; i < mapJobs; i++ {
		...
		cmp := protocol.Message{
			...
			Input: s.input[i*inputSize : int(math.Min(float64(totalInputSize), float64((i+1)*inputSize)))],
			Code:  s.mapCode,
		}
		if !s.Equal(&cmp, msg) {
			return
	...
	go func() {
		// Ticking is necessary because this loop doesn't do any IO
		// or channel operations, thus not calling the runtime scheduler,
		// which starves all other goroutines.
	...
	//
	// Mock shuffled reduce inputs to compare against.
	unshuffled := make([][]protocol.Input, reduceJobs)
	for i := 0; i < mapJobs; i++ {
		for index, outputs := range test.MockMapJobOutput(totalInputSize, mockMapOutputSize, []int{0, 2, 4}) {
			unshuffled[index] = append(unshuffled[index], outputs)
		}
	}

	// Mock a single partition reduce output.
	shuffled := make(map[string][]string)
	for _, mapOutputs := range unshuffled {
		// Pick the first non-empty partition and shuffle it.
		// We'll use the shuffled result for to mock the other partitions as well.
		if len(mapOutputs) > 0 {
			for _, outputs := range mapOutputs {
				for _, output := range outputs {
					shuffled[output.K] = append(shuffled[output.K], *output.V)
				}
			}

			// No need to shuffle the rest of the partitions,
	...
	for i := 0; i < reduceJobs; i++ {
		...
		// Mock reduce input with shuffled map outputs.
		partitions[i] = make(protocol.Input, 0)
		if contains([]int{0, 2, 4}, i) {
			for k, vs := range shuffled {
				partitions[i] = append(partitions[i], protocol.NewReduceInputValue(k, vs))
			}
		}
		...
		s.Require().Equal(cmp.Meta, msg.Meta)
		s.Require().Equal(cmp.Code, msg.Code)

		// Inputs are not equal, so we compare the length.
		// This is because the partition index is written as the 5th MSB digit
		// in each input value. See MockMapJobOutput() for more details.
		s.Require().Len(msg.Input, len(cmp.Input))
...
// contains is a helper function which returns false
// if the given int list contains given int.
