...
// BaseInitializer implements the Initializer interface,
// and can initialize basic workers.
type BaseInitializer struct{}

// NewWorker returns a new basic worker.
...
func (b *BaseInitializer) NewWorker() (Worker, error) { return newBaseWorker() }

// BaseWorker implements the Worker interface, but without network communication.
// It doesn't perform any "real" MapReduce work,
// besides being able to send and receive messages via its channels.
// It should be embedded in every other "real" worker, e.g. SockJSWorker.
//
// It is also used in unit tests as it can mock a real worker.
...
func newBaseWorker() (Worker, error) {
	w := BaseWorker{
